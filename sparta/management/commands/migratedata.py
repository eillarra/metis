...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.out("info", "Migrating faculties...")

        for obj in session.query(Base.classes.Faculty).all():
            base = get_base_fields(obj, "Faculty")
        ...
        self.out("success", f"✔ Faculties migrated! ({Faculty.objects.count()} records)")
        ...
        self.out("info", "Migrating disciplines...")
        ...
        self.out("info", "Migrating projects...")

        for obj in session.query(Base.classes.TrainingProject).all():
            base = get_base_fields(obj, "TrainingProject")
            Project.objects.create(
                **base,
                id=obj.TrainingProjectID,
                name=obj.TrainingProjectName,
            )

        self.out("success", f"✔ Projects migrated! ({Project.objects.count()} records)")
