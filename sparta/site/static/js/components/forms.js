var SpartaFormComponents = {
  ...
  'sparta-editor': {
      ...
      showDialog: {
        ...
        set: function (val) {
          if (this.dialogVisible) {
            ...
            EventEmitter.emit('sparta-editor-hide');
          }
        }
      }
    },
    ...
    created: function () {
      EventEmitter.on('sparta-editor-hide-after-save', this.hideDialog);
    },
    beforeUnmount: function () {
      EventEmitter.off('sparta-editor-hide-after-save');
    }
  },
      ...
      <div class="q-mb-lg">
        ...
        <div v-for="el in stack" class="row q-col-gutter-xs q-mb-sm items-center">
          ...
          <div class="col-1 text-center">
            <sparta-remove-icon @click.prevent="removeFromStack(el)" />
          </div>
        </div>
  ...
  'sparta-country-select': {
    emits: ['update:modelValue'],
    data: function () {
      return {
        storageKey: 'sparta_countries',
        countries: null,
        mutable: null
      };
    },
    ...
    methods: {
      getCountries: function () {
        ...
        Sparta.api.request('get', '/api/countries/').then(function (res) {
...
<!-- text_list -->
              <sparta-text-list v-else-if="field.type == 'text_list'" v-model="mutable.custom_data[field.id]" :fields="field.fields" :limit="field.limit" />
<!-- single_choice || multiple_choice || variable_choice -->
              <sparta-choice-field v-else-if="field.type == 'single_choice' || field.type == 'multiple_choice'" v-model="mutable.custom_data[field.id]" :options="field.options" :multiple="field.type == 'multiple_choice'" :autocomplete="field.autocomplete" :vary="!!field.vary_on" :vary-on="mutable.custom_data[field.vary_on]" />
<!-- checkbox -->
...
<!-- date -->
              <sparta-datepicker v-else-if="field.type == 'date'" v-model="mutable.custom_data[field.id]" :label="field.label" :options="field.options" />
<!-- [other] -->
    ...
    computed: {
      topicsSorted: function () {
        return this.event.topics.sort(function (a, b) {
          return Sparta.utils.sortText(a.name, b.name);
        });
      },
