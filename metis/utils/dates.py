...
def is_holiday(date: date, country_code: str = "BE") -> bool:
    ...
    :returns: A boolean indicating whether or not the given date is a holiday in the given country.
    """
...
def is_weekend(date: date) -> bool:
    ...
    :param date: The date to check.
    :returns: A boolean indicating whether or not the given date is in the weekend.
    """
...
def remind_deadline(moment: datetime, deadline: datetime, remind_before: list[int] | None = None) -> bool:
    ...
    :param remind_before: The number of days before the deadline to remind. Defaults to [0, 3].
    :returns: A boolean indicating whether or not the deadline should be reminded.
    """
    ...
    return (deadline_date - date).days in remind_before and not (is_weekend(date) or is_holiday(date))
...
def get_time_difference(time1: time, time2: time) -> time:
    ...
    :param time1: The first time.
    :param time2: The second time.
    :returns: The time difference between the two times.
    """
...
def sum_times(times: list[time]) -> tuple[int, int]:
    ...
    :returns: The sum of the given times, as a tuple of hours and minutes.
    """
    hours = 0
    minutes = 0
