  ...
  'metis-locale-menu': {
        ...
        <a class="cursor-pointer">
          ...
          <q-menu anchor="top end" self="bottom right" :offset="[0, 8]">
            <q-list dense style="min-width: 100px" class="text-body2 q-py-sm">
              <q-item v-for="l in locales" clickable :disable="l.code == locale" @click="change(l.code)">
                <q-item-section >{{ l.name }}</q-item-section>
              </q-item>
            </q-list>
          </q-menu>
        </a>
        <form ref="localeForm" :action="actionUrl" method="post">
    ...
    methods: {
      change: function (lang) {
        this.$refs.localeForm.language.value = lang;
    ...
    methods: {
      copyToClipboard: function () {
        Quasar.copyToClipboard(this.text).then(function () {
          Metis.utils.notify('Copied to clipboard', 'none');
        }).catch(function () {
          Metis.utils.notify('Could not copy to clipboard', 'warning');
        });
      }
    }
  },

  'metis-contact-dialog': {
    data: function () {
      return {
        dialogVisible: false,
        user: null,
    ...
    methods: {
      ...
      sendMessage: function () {
        Metis.api.request('post', this.eventUrl + 'contact/', {
          user_id: this.user.id,
          message: this.msg
        }).then(function (res) {
          Metis.utils.notify('Message sent.');
        }).catch(function (error) {
          Metis.utils.notifyApiError(error);
        });
      }
    },
    created: function () {
