<template>
  <div>
    ...
    <p>U bent aangemeld als contactpersoon voor de stageplaats <strong>{{ place.name }}</strong>.</p>
    ...
    <p class="bg-yellow-2"><strong>In een eerste stap</strong> wensen we de accounts voor METIS te actualiseren. Daarom vragen we u vriendelijk om onderstaande info aandachtig te lezen, het e-mailbericht aan te vullen en meteen te versturen.</p>
    ...
    <div class="row q-col-gutter-xl q-mb-xl">
      <div class="col-12 col-md">
        ...
        <p>De admin (één person) kan de informatie van de stageplaats (kledij, patiëntenpopulatie, bereikbaarheid, werkrooster, …) rechtstreeks in METIS aanpassen. Dus niet meer op papier en via e-mail zoals voorheen.</p>
      </div>
      <div class="col-12 col-md">
        <h5 class="q-mb-md">Ontbrekende collega's?</h5>
        <p>Zijn er nog collega's die ontbreken in bovenstaande lijst. Gelieven hun naam en e-mailadres door te geven. We sturen hen dan ook een uitnodiging om een account aan te maken.</p>
      </div>
    </div>
  </div>
</template>
...
function sendEmail() {
  const url = '/user/contact/places/' + place.value.id + '/email/';
  const data = {
    text: email.value,
  };
...
let text = `De contactpersonen voor stageplaats ${place.value.name} zijn:\n\n`;
for (const contact of place.value.contacts) {
