<template>
  <dialog-form icon="add" :title="$t('student')">
    <template #page>
      <q-stepper v-model="step" vertical flat color="primary" animated keep-alive>
        <q-step :name="1" :title="$t('form.student.create.add_existing')" icon="search" active-icon="search">
          {{ $t('form.student.create.search') }}
          <div class="q-gutter-sm q-mt-sm">
            <api-autocomplete
              v-model="formData.user"
              clearable
              search-endpoint="/users/"
              :mapper="userMapper"
              :label="$t('contact')"
            />
            <program-block-select :programs="programs" v-model="formData.block_id" class="col-12 col-md-2" />
          </div>
          <q-stepper-navigation class="flex">
            <q-btn
              unelevated
              @click="step = 2"
              color="blue-1"
              text-color="ugent"
              :label="$t('form.skip')"
              :disable="!!formData.user"
            />
            <q-space />
            <q-btn
              unelevated
              @click="addStudent"
              color="primary"
              :label="$t('form.add_to_project')"
              :disable="!formData.user || !formData.block_id"
            />
          </q-stepper-navigation>
        </q-step>
        <q-step :name="2" :title="$t('form.student.create.new')" icon="mail_outline" active-icon="mail_outline">
          {{ $t('form.student.create.invite') }}
          <div class="q-gutter-sm q-mt-sm">
            <q-input v-model="formData.name" dense :label="$t('field.name')" type="text" />
            <q-input v-model="formData.email" dense :label="$t('field.email')" type="email" />
            <program-block-select :programs="programs" v-model="formData.block_id" class="col-12 col-md-2" />
          </div>
        </q-step>
      </q-stepper>
    </template>
    <template #footer>
      <div v-if="step == 2" class="flex q-gutter-sm q-pa-lg">
        <q-btn unelevated @click="step = 1" color="blue-1" text-color="ugent" :label="$t('form.back')" />
        <q-space />
        <q-btn
          unelevated
          @click="inviteStudent"
          color="primary"
          :label="$t('form.invite')"
          :disable="!formData.name || !formData.email || !formData.block_id"
        />
      </div>
    </template>
  </dialog-form>
</template>

<script setup lang="ts">
import { computed, ref } from 'vue';
import { storeToRefs } from 'pinia';
import { useI18n } from 'vue-i18n';

import { api } from '@/axios';
import { notify } from '@/notify';

import { useOfficeStore } from '../../store.js';

import ApiAutocomplete from '@/components/forms/ApiAutocomplete.vue';
import DialogForm from '@/components/forms/DialogForm.vue';
import ProgramBlockSelect from '../ProgramBlockSelect.vue';

const emit = defineEmits(['create:obj']);

const { t } = useI18n();
const officeStore = useOfficeStore();
const { education, programs, project, projectStudents } = storeToRefs(officeStore);

const step = ref(1);
const formData = ref({
  user: null as QuasarAutocompleteOption | null,
  block_id: null as number | null,
  name: null as string | null,
  email: null as string | null,
});

function userMapper(data: ApiObject[]) {
  return data.map((obj) => ({
    id: obj.id,
    name: (obj as UserTiny).name,
    caption: (obj as UserTiny).email,
    disable: userIdsUsedByProjectStudents.value.has(obj.id),
  }));
}

function addStudent() {
  if (!project.value || !formData.value.user) return;

  const data = {
    user_id: formData.value.user.id,
    block_id: formData.value.block_id,
  };

  api.post(`${project.value.self}students/`, data).then((res) => {
    officeStore.createObj('student', res.data);
    notify.success(t('form.student.create.success'));
    emit('create:obj');
  });
}

function inviteStudent() {
  console.log(education.value, formData.value);
}

/**
 * Returns a set of user IDs used by the students of the current project.
 * @returns {Set<number>}
 */
const userIdsUsedByProjectStudents = computed<Set<number>>(() => {
  return projectStudents.value.reduce((set, obj) => {
    set.add(obj.user);
    return set;
  }, new Set<number>());
});
</script>
