  ...
  <dialog-form icon="person" :title="student.name">
      ...
      <q-tab-panels v-model="tab" class="q-px-sm">
        <q-tab-panel name="info">
          <div class="q-gutter-sm">
            <div class="row q-col-gutter-lg q-pl-sm">
              <q-input v-model="obj.project.name" :label="$t('project')" readonly class="col-12 col-md" />
              <q-input v-model="obj.block.name" :label="$t('program_block')" readonly class="col-12 col-md" />
            </div>
            <q-input v-model="student.name" :label="$t('student')" readonly />
            <q-input v-model="student.email" :label="$t('field.email')" readonly />
          </div>
        </q-tab-panel>
    ...
    <template #footer>
      <div v-if="tab == 'info'" class="flex q-gutter-sm q-pa-lg">
        ...
        />
        <q-space />
        <!--<q-btn @click="save" unelevated color="primary" :label="$t('form.student.save')" />-->
      </div>
    </template>
  </dialog-form>
</template>
