export default {
  ugent: 'Universiteit Gent',
  ge: 'Faculteit Geneeskunde en Gezondheidswetenschappen',
  version: 'Versie',
  loading: 'Aan het laden...',
  user_menu: {
    dashboard: 'Mijn dashboard',
    login: 'Aanmelden',
    logout: 'Afmelden',
  },
  home: {
    presentation:
      'Digitaal platform voor de communicatie, organisatie en evaluatie van stages binnen de faculteit Geneeskunde en Gezondheidswetenschappen',
    oauth_login: 'Aanmelden met UGent account',
    help: {
      no_account: 'Geen UGent account?',
    },
  },
  contact: 'Contact | Contacten',
  contact_only: 'Enkel een contactpersoon',
  discipline: 'Discipline | Disciplines',
  mentor: 'Mentor | Mentoren',
  internship: 'Stage | Stages',
  period: 'Periode | Periodes',
  planning: 'Stageplanning',
  profile: 'Profiel | Profielen',
  program: "Programma | Programma's",
  project: 'Project | Projecten',
  program_block: 'Blok | Blokken',
  program_internship: 'Stage | Stages',
  place: 'Stageplaats | Stageplaatsen',
  region: "Regio | Regio's",
  remark: 'Opmerking | Opmerkingen',
  staff: 'Staff | Staffleden',
  student: 'Student | Studenten',
  text: 'Tekst | Teksten',
  track: 'Traject | Trajecten',
  type: 'Type | Types',
  field: {
    email: 'Email | Emails',
    name: 'Naam | Namen',
    code: 'Interne code | Interne codes',
    date: 'Datum | Data',
    place_type: 'Type stageplaats',
    start_date: 'Startdatum',
    end_date: 'Einddatum',
  },
  form: {
    new: 'Nieuw',
    back: 'Terug',
    continue: 'Verdergaan',
    skip: 'Overslaan',
    invite: 'Uitnodigen',
    add_to_place: 'Voeg toe aan stageplaats',
    add_to_project: 'Voeg toe aan project',
    contact: {
      create: {
        new: 'Nieuwe contactpersoon aanmaken',
        add_existing: 'Bestaande account toevoegen',
        search: 'Als de contactpersoon een account heeft, kan je deze hier toevoegen. Als er geen account gevonden wordt, kan je de contactpersoon uitnodigen om er een aan te maken.',
        invite: 'Er zal een email verstuurd worden naar de contactpersoon om hen uit te nodigen om een account aan te maken. Ze zullen aan het project toegevoegd worden nadat ze een account hebben aangemaakt.',
        success: 'Contactpersoon aangemaakt',
      },
      delete: 'Contactpersoon verwijderen',
      confirm_delete: 'Ben je zeker dat je deze contactpersoon wil verwijderen?',
      deleted: 'Contactpersoon verwijderd',
      save: 'Contactpersoon opslaan',
      saved: 'Contactpersoon informatie opgeslagen',
    },
    internship: {
      create: {
        new: 'Nieuwe stage aanmaken',
        success: 'Stage aangemaakt',
      },
      delete: 'Stage verwijderen',
      confirm_delete: 'Ben je zeker dat je deze stage wil verwijderen?',
      deleted: 'Stage verwijderd',
      confirm_delete_student: 'Ben je zeker dat je deze student wil verwijderen uit deze stage?',
      deleted_student: 'Student verwijderd uit stage',
      save: 'Stage opslaan',
      saved: 'Stage informatie opgeslagen',
    },
    place: {
      create: {
        new: 'Nieuwe stageplaats aanmaken',
        add_existing: 'Bestaande stageplaats toevoegen',
        search:
          'Als de stageplaats eerder is aangemaakt voor een ander project, kan je deze hier toevoegen. Als er geen stageplaats gevonden wordt, kan je een nieuwe stageplaats aanmaken.',
        success: 'Stageplaats aangemaakt',
      },
      delete: 'Stageplaats verwijderen',
      confirm_delete: 'Ben je zeker dat je deze stageplaats wil verwijderen?',
      deleted: 'Stageplaats verwijderd',
      save: 'Stageplaats opslaan',
      saved: 'Stageplaats informatie opgeslagen',
    },
    student: {
      create: {
        new: 'Nieuwe student aanmaken',
        add_existing: 'Bestaande student toevoegen',
        search: 'Als de student een account heeft, kan je deze hier toevoegen. Als er geen account gevonden wordt, kan je de student uitnodigen om er een aan te maken.',
        invite: 'Er zal een email verstuurd worden naar de student om hen uit te nodigen om een account aan te maken. Ze zullen aan het project toegevoegd worden nadat ze een account hebben aangemaakt.',
        success: 'Student aangemaakt',
      },
      delete: 'Student verwijderen',
      confirm_delete: 'Ben je zeker dat je deze student wil verwijderen?',
      deleted: 'Student verwijderd',
      save: 'Student opslaan',
      saved: 'Student informatie opgeslagen',
    },
  },
  copied_to_clipboard: 'Gekopieerd naar het klembord',
};
