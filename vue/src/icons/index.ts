import {
  symSharpAccountBox,
  symSharpAccountTree,
  symSharpAdd,
  symSharpAdsClick,
  symSharpAttachFile,
  symSharpBackspace,
  symSharpCalendarMonth,
  symSharpChatBubble,
  symSharpCheck,
  symSharpChecklist,
  symSharpClose,
  symSharpDocsAddOn,
  symSharpDomainVerification,
  symSharpDoneOutline,
  symSharpDownload,
  symSharpContacts,
  symSharpContentCopy,
  symSharpCorporateFare,
  symSharpDateRange,
  symSharpEast,
  symSharpEdit,
  symSharpFilter1,
  symSharpFilter2,
  symSharpFilter3,
  symSharpFilter4,
  symSharpFilter5,
  symSharpFolderOpen,
  symSharpGroup,
  symSharpQuestionMark,
  symSharpHideSource,
  symSharpHistoryToggleOff,
  symSharpInfo,
  symSharpInventory,
  symSharpKeyboardArrowUp,
  symSharpLocationOn,
  symSharpLogout,
  symSharpMail,
  symSharpMap,
  symSharpMarkChatUnread,
  symSharpMarkEmailRead,
  symSharpMoreTime,
  symSharpNumbers,
  symSharpOpenInBrowser,
  symSharpPendingActions,
  symSharpPlaylistAddCheck,
  symSharpRadioButtonChecked,
  symSharpRadioButtonUnchecked,
  symSharpSchedule,
  symSharpSearch,
  symSharpSelectCheckBox,
  symSharpSettings,
  symSharpSettingsAccountBox,
  symSharpSignature,
  symSharpSmartToy,
  symSharpSplitscreenLeft,
  symSharpStackedEmail,
  symSharpStylusNote,
  symSharpVisibility,
  symSharpWarning,
} from '@quasar/extras/material-symbols-sharp';

export const iconAccount =
  'M222-255q63-40 124.5-60.5T480-336q72 0 134 20.5T739-255q44-54 62.5-109T820-480q0-145-97.5-242.5T480-820q-145 0-242.5 97.5T140-480q0 61 19 116t63 109Zm257.81-195q-57.81 0-97.31-39.69-39.5-39.68-39.5-97.5 0-57.81 39.69-97.31 39.68-39.5 97.5-39.5 57.81 0 97.31 39.69 39.5 39.68 39.5 97.5 0 57.81-39.69 97.31-39.68 39.5-97.5 39.5Zm-.21 370q-83.15 0-156.28-31.5t-127.22-86Q142-252 111-324.84 80-397.68 80-480.5t31.5-155.66Q143-709 197.5-763t127.34-85.5Q397.68-880 480.5-880t155.66 31.5Q709-817 763-763t85.5 127Q880-563 880-480.27q0 82.74-31.5 155.5Q817-252 763-197.5t-127.13 86Q562.74-80 479.6-80Z|0 -960 960 960';
export const iconActions = symSharpAdsClick;
export const iconAdd = symSharpAdd;
export const iconAddBox =
  'M440-280h80v-160h160v-80H520v-160h-80v160H280v80h160v160ZM120-120v-720h720v720H120Z|0 -960 960 960';
export const iconAttachment = symSharpAttachFile;
export const iconCalendar = symSharpCalendarMonth;
export const iconCalendarRange = symSharpDateRange;
export const iconChat = symSharpChatBubble;
export const iconChatBadge = symSharpMarkChatUnread;
export const iconCheck = symSharpCheck;
export const iconCheckDouble = symSharpDoneOutline;
export const iconCheckbox =
  'm424-312 282-282-56-56-226 226-114-114-56 56 170 170ZM120-120v-720h720v720H120Z|0 -960 960 960';
export const iconClose = symSharpClose;
export const iconColumns = symSharpSplitscreenLeft;
export const iconContact = symSharpAccountBox;
export const iconContacts = symSharpContacts;
export const iconCopy = symSharpContentCopy;
export const iconDelete = symSharpBackspace;
export const iconDownload = symSharpDownload;
export const iconDraft = symSharpStylusNote;
export const iconEdit = symSharpEdit;
export const iconEmail = symSharpMail;
export const iconEmailPrimary = symSharpMarkEmailRead;
export const iconEmailWarning = 'M731-441 502-40h458Zm-20 121h40v160h-40zm19 200h1c5.3 0 10 2 14 6a19.2 19.2 0 0 1 0 28 19.2 19.2 0 0 1-28 0 19.2 19.2 0 0 1 0-28c3.8-3.7 8-5.8 13-6zM80-800v640h404.6l45.7-80H160v-400l320 200 320-200v172.3l80 140.1V-800Zm80 80h640L480-520z|0 -960 960 960';
export const iconEmails = symSharpStackedEmail;
export const iconEmailSettings =
  'm720-440-12.031 60c-8 3.333-15.5 6.802-22.5 10.469-7 3.666-14.151 8.182-21.485 13.515l-57.968-17.968-40 67.968 45.937 40c-1.333 8-1.953 16.683-1.953 26.016 0 9.333.62 18.016 1.953 26.016l-45.937 40 40 67.968 57.968-17.968c7.334 5.333 14.485 9.849 21.485 13.515 7 3.667 14.5 7.136 22.5 10.469L720-40h80l11.953-60c8-3.333 15.5-6.802 22.5-10.469 7-3.666 14.23-8.182 21.563-13.515l57.968 17.968 40-67.968-46.015-40c1.333-8 2.031-16.683 2.031-26.016 0-9.333-.698-18.016-2.031-26.016l46.015-40-40-67.968-57.968 17.968c-7.334-5.333-14.563-9.849-21.563-13.515-7-3.667-14.5-7.136-22.5-10.469L800-440Zm40 120c22 0 40.818 7.849 56.484 23.516C832.151-280.818 840-262 840-240s-7.849 40.818-23.516 56.484C800.818-167.849 782-160 760-160s-40.818-7.849-56.484-23.516C687.849-199.182 680-218 680-240s7.849-40.818 23.516-56.484C719.182-312.151 738-320 760-320ZM80-800v640h412.031c-4-13.333-7.062-26.531-9.062-39.531-2-13-2.969-26.469-2.969-40.469H160v-400l320 200 320-200v123.984c14 2 27.818 4.985 41.484 8.985 13.667 4 26.516 9 38.516 15V-800Zm80 80h640L480-520z|0 -960 960 960';
export const iconEye = symSharpVisibility;
export const iconExpandLess = symSharpKeyboardArrowUp;
export const iconHelp = symSharpQuestionMark;
export const iconInfo = symSharpInfo;
export const iconLocation = symSharpLocationOn;
export const iconLogout = symSharpLogout;
export const iconMap = symSharpMap;
export const iconMicrosoft = symSharpSettingsAccountBox;
export const iconMentors = symSharpContacts;
export const iconNumbers = symSharpNumbers;
export const iconOpenDialog = symSharpOpenInBrowser;
export const iconRadioChecked = symSharpRadioButtonChecked;
export const iconRadioUnchecked = symSharpRadioButtonUnchecked;
export const iconRobot = symSharpSmartToy;
export const iconSearch = symSharpSearch;
export const iconSettings = symSharpSettings;
export const iconSignature = symSharpSignature;
export const iconTime = symSharpSchedule;
export const iconTimeDashed = symSharpHistoryToggleOff;

export const iconStep1 = symSharpFilter1;
export const iconStep2 = symSharpFilter2;
export const iconStep3 = symSharpFilter3;
export const iconStep4 = symSharpFilter4;
export const iconStep5 = symSharpFilter5;
export const iconStepFinal =
  'm490-383 228-228-42-41-185 186-97-97-42 42 138 138ZM200-200v-680h680v680H200ZM80-80v-680h60v620h620v60H80Z|0 -960 960 960';

export const iconNotifyError = symSharpClose;
export const iconNotifyOk = symSharpCheck;
export const iconNotifyInfo = symSharpInfo;
export const iconNotifyWarning = symSharpWarning;

export const iconTableBoolTrue =
  'm424-296 282-282-56-56-226 226-114-114-56 56 170 170Zm56 216q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Z|0 -960 960 960';
export const iconTableBoolFalse = symSharpHideSource;
export const iconTableCheck = symSharpCheck;
export const iconTableCheckPending = symSharpRadioButtonChecked;

export const iconUgentArrowRight = symSharpEast;

export const iconEvaluation = symSharpPlaylistAddCheck;
export const iconEvaluations = symSharpChecklist;
export const iconFiles = symSharpFolderOpen;
export const iconInternship = symSharpDateRange;
export const iconPlace = symSharpCorporateFare;
export const iconProgram = symSharpAccountTree;
export const iconQuestioning = symSharpSelectCheckBox;
export const iconResponses = symSharpInventory;
export const iconResponsesPending = symSharpPendingActions;
export const iconStudent = symSharpGroup;
export const iconTasks = symSharpDomainVerification;
export const iconTimesheet = symSharpSchedule;

export const iconEvaluationAdd = symSharpDocsAddOn;
export const iconTimesheetAdd = symSharpMoreTime;
