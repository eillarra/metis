...
from epione.models.rel.files import File, FilesMixin
...
class Case(FilesMixin, RemarksMixin, BaseModel):
    """
    `reviewer` is a User in Epione. it can be suggested by the student, in that case a new account is created.
    ...
    internship = models.ForeignKey("epione.Internship", related_name="cases", on_delete=models.CASCADE)
    reviewer = models.ForeignKey("epione.User", null=True, related_name="cases", on_delete=models.SET_NULL)
    ...
    class Meta:
        db_table = "epione_internship_case"
