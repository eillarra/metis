...
var EpioneCommonComponents = {
  ...
  'epione-locale-menu': {
    data: function () {
      return {
        csrfToken: Epione.csrf.getToken(),
        locales: [
    ...
    methods: {
      copyToClipboard: function () {
        Quasar.copyToClipboard(this.text).then(function () {
          Epione.utils.notify('Copied to clipboard', 'none');
        }).catch(function () {
          Epione.utils.notify('Could not copy to clipboard', 'warning');
        });
      }
    }
  },

  'epione-contact-dialog': {
    data: function () {
      return {
        dialogVisible: false,
        user: null,
    ...
    methods: {
      ...
      sendMessage: function () {
        Epione.api.request('post', this.eventUrl + 'contact/', {
          user_id: this.user.id,
          message: this.msg
        }).then(function (res) {
          Epione.utils.notify('Message sent.');
