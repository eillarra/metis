...
var Epione = {
  ...
  api: {
    request: function (method, url, data, headers) {
      var headers = headers || {};
      headers['Accept-Language'] = DJANGO_VARS.language;
      headers["X-CSRFTOKEN"] = Epione.csrf.getToken();
    ...
    create: function (url, obj, callbackFn, customMsg) {
      this.request('post', url, obj).then(function (res) {
        ...
        Epione.utils.notify((customMsg) ? customMsg : modelFromUrl(res.data.self || url) + ' created.');
      }).catch(function (error) {
        Epione.utils.notifyApiError(error);
      });
    },
    update: function (obj, callbackFn, customMsg) {
      this.request('put', obj.self, obj).then(function (res) {
        if (callbackFn) callbackFn(res);
        Epione.utils.notify((customMsg) ? customMsg : modelFromUrl(obj.self) + ' updated.');
      }).catch(function (error) {
        Epione.utils.notifyApiError(error);
      });
    },
    remove: function (obj, callbackFn, customMsg) {
      this.request('delete', obj.self).then(function (res) {
        if (callbackFn) callbackFn(res);
        Epione.utils.notify((customMsg) ? customMsg : modelFromUrl(obj.self) + ' deleted.');
      }).catch(function (error) {
        Epione.utils.notifyApiError(error);
      });
    }
  ...
  map: {
    abstract: function (obj) {
      ...
      Epione.map.qBooleans(obj, [
        ['accepted', 'is_accepted'],
      ]);

      Epione.map.qCustomData(obj);
  ...
  utils: {
    ...
    confirmAction: function (msg, okCallbackFn, cancelCallbackFn) {
      Quasar.Dialog.create({
        message: msg,
        class: 'epione-confirm-dialog q-pa-sm',
        focus: 'none',
        cancel: {
          'flat': true,
          'color': 'grey-8'
